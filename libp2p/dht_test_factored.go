...
func TestSendDHTUpdate(t *testing.T) {
	...
	var p2p1, p2p2 P2P
	p2p1.NewHost(ctx, priv1, true)
	p2p2.NewHost(ctx, priv2, true)

	defer p2p1.Host.Close()
	defer p2p2.Host.Close()

	p2p1.SetStreamHandler(DHTProtocolID, func(s network.Stream) {
		...
		p2p1.Host.Peerstore().Put(peerID, "peer_info", peerInfo)
	})
	p2p2.SetStreamHandler(DHTProtocolID, func(s network.Stream) {
		peerInfo := models.PeerData{}
		var peerID peer.ID
		data, err := io.ReadAll(s)
		if err != nil {
		...
		p2p2.Host.Peerstore().Put(peerID, "peer_info", peerInfo)
	})

	err := p2p1.BootstrapNode(ctx)
	if err != nil {
		t.Errorf("Expected Bootstrap to succeed but got error: %v", err)
	}
	err = p2p2.BootstrapNode(ctx)
	if err != nil {
		t.Errorf("Expected Bootstrap to succeed but got error: %v", err)
	}

	p2p1.StartDiscovery(ctx, CIRendevousPoint)
	p2p2.StartDiscovery(ctx, CIRendevousPoint)

	//Connect to each other
	if err := p2p1.Host.Connect(ctx, p2p2.Host.Peerstore().PeerInfo(p2p2.Host.ID())); err != nil {
		t.Fatalf("Unable to connect ---- %v ", err)
	}

	time.Sleep(1 * time.Second)

	for p2p1.Host.Network().Connectedness(p2p2.Host.ID()).String() != "Connected" {
		if err := p2p1.Host.Connect(ctx, p2p2.Host.Peerstore().PeerInfo(p2p2.Host.ID())); err != nil {
	...
	peerInfo1.PeerID = p2p1.Host.ID().String()
	peerInfo1.HasGpu = true
	peerInfo1.AllowCardano = false
	// p2p1.Host.Peerstore().Put(p2p1.Host.ID(), "peer_info", peerInfo1)
	peerInfo2 := models.PeerData{}
	peerInfo2.PeerID = p2p2.Host.ID().String()
