...
var (
	flagCudaTensor, flagRocmHip bool
)

func NewGPUCapacityCmd(librarier Librarier, docker Docker) *cobra.Command {
	cmd := &cobra.Command{
		Use:    "capacity",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cuda {
				...
				if !imageExists(images, cudaOpts.Image) {
					err := pullImage(ctx, docker, cudaOpts.Image, cmd.OutOrStdout())
					if err != nil {
						return fmt.Errorf("failed to pull CUDA image %s: %w", cudaOpts.Image, err)
					}
				}

				err = runDockerContainer(ctx, docker, cudaOpts, cmd.OutOrStderr())
				if err != nil {
			...
			if rocm {
				...
				if !imageExists(images, rocmOpts.Image) {
					err := pullImage(ctx, docker, rocmOpts.Image, cmd.OutOrStdout())
					if err != nil {
						return fmt.Errorf("could not pull ROCm-HIP image %s: %w", rocmOpts.Image, err)
					}
				}

				err = runDockerContainer(ctx, docker, rocmOpts, cmd.OutOrStderr())
				if err != nil {
	...
	cmd.Flags().BoolVarP(&flagCudaTensor, "cuda-tensor", "c", false, "check CUDA Tensor")
	cmd.Flags().BoolVarP(&flagRocmHip, "rocm-hip", "r", false, "check ROCM-HIP")

	return cmd
}

func runDockerContainer(ctx context.Context, docker Docker, options ContainerOptions, w io.Writer) error {
	var (
		err        error
		id         string
	...
	id, err = createAndStartContainer(ctx, docker, config, hostConfig, w)
	if err != nil {
		return fmt.Errorf("failed to create and start container %s: %w", id, err)
	}

	err = attachAndMonitorContainer(ctx, docker, id, w)
	if err != nil {
...
func validateOptions(options ContainerOptions) error {
	...
	if len(options.Command) == 0 {
		return fmt.Errorf("command field cannot be empty")
	}

	if len(options.Entrypoint) == 0 {
		return fmt.Errorf("entrypoint field cannot be empty")
	}
...
func createAndStartContainer(ctx context.Context, docker Docker, config *container.Config, hostConfig *container.HostConfig, w io.Writer) (string, error) {
	var (
...
func attachAndMonitorContainer(ctx context.Context, docker Docker, id string, w io.Writer) error {
	var (
	...
	out, err = docker.ContainerAttach(ctx, id, types.ContainerAttachOptions{
		Stream: true,
		Stdout: true,
		Stderr: true,
	})
	...
	waitCh, errCh = docker.ContainerWait(ctx, id, container.WaitConditionNotRunning)
	select {
...
func pullImage(ctx context.Context, docker Docker, image string, w io.Writer) error {
	ctxCancel, cancel := context.WithCancel(ctx)
	defer cancel()

	out, err := docker.ImagePull(ctxCancel, image, types.ImagePullOptions{})
	if err != nil {
