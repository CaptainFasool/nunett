...
func (l *Libp2p) DiscoverDialPeers(ctx context.Context) error {
	foundPeers, err := l.fingPeersFromRendezvousDiscovery(ctx)
	if err != nil {
		return err
	}
...
// fingPeersFromRendezvousDiscovery uses the randevouz point to discover other peers.
func (l *Libp2p) fingPeersFromRendezvousDiscovery(ctx context.Context) ([]peer.AddrInfo, error) {
